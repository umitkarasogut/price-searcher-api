...
export default class HepsiburadaProvider extends AbstractProvider implements Provider {
    ...
    public static processHtml(html: string): Product | Product[] {
        ...
        const products = ([...document.querySelectorAll('ul.productListContent-wrapper li.productListContent-item')] as Element[]).map(element => ({
            ...
            url: HepsiburadaProvider.url + element.querySelector('a')?.getAttribute('href')!,
            image: HepsiburadaProvider.url + element.querySelector("[data-test-id='product-image-image'] > pricture > source")?.getAttribute('srcset')!, //TODO:this not crawling
        }));

        console.log(`%cProcessed ${products.length} ${HepsiburadaProvider.providerName} product`, "color:blue");
